  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Subscribes to real-time changes on the database
        channel = supabase.channel('crm-realtime')
          .on(
            'postgres_changes',
            { event: 'UPDATE', schema: 'public', table: 'crm_data', filter: 'id=eq.1' },
            (payload) => {
              const { new: newData } = payload;
              if (newData && active) {
                // If a change happens on another client, this will sync it instantly.
